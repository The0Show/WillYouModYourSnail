	...
	async InstallMod(file) {
		try {
			...
			if (file.name.endsWith(".zip")) {
				...
				try {
					const tempId = Buffer.from(Date.now().toString(), "utf8").toString(
						"base64url"
					);
					...
					if (modMetadataJSON.id === undefined) throw new Error("No id!");
					...
					const modBinary = glob.sync(
						`${this.appData.replaceAll("\\", "/")}/Temp/${tempId}/**/*.dll`,
						{}
					);
					...
					if (header !== "MZ") throw new Error("Mod binary is not DLL");
					...
					const modDebugger = glob.sync(
						`${this.appData.replaceAll("\\", "/")}/Temp/${tempId}/**/*.pdb`,
						{}
					);
					...
					const modCover = glob.sync(
						`${this.appData.replaceAll("\\", "/")}/Temp/${tempId}/**/cover.png`,
						{}
					);
				...
				this.sendUploadResponse(
					`Patch mods are not supported or recommended.<br />Ask the developer to make a GMML version (nicely)!`,
					"danger"
				);
			} else {
